...
export interface AirpayHandoff {
   ...
   * sent by a client is ignored. Airpay resolves its Response URL per MID from
   * its own dashboard, so for merchant 2 — whose dashboard points elsewhere —
   * the answer is no, and this tab must not be navigated away or the shopper
   * never comes back.
   *
  ...
  try {
    const response = await fetch('/api/payments/create', {
      ...
      body: JSON.stringify({
        ...
        // No MID, credential, order reference or gateway URL is sent — the
        // browser holds none of them.
 ...
 * for merchant 1 — whose dashboard Response URL points back at this site, so
 * Airpay itself brings the shopper home to /order-success (§8.1, §14.3).
 */
