...
function seedOrder(ref: string, overrides: Record<string, unknown> = {}): void {
  orders.map.set(ref, {
    ...
    // ⚠ AFTER the legacy cutoff. These are NEW orders from the re-enabled
    // merchant-2 checkout option, not the six historical AM2- records of the
    // retired experiment — which are covered in api/_lib/legacy-order-ref.test.ts
    // and are the only AM2- rows settlement declines to act on.
