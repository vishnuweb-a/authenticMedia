 ...
 *
 * MID 362380 registers this URL in its own Airpay dashboard as BOTH its
 * Response URL and its IPN URL, so Airpay delivers to KKChat directly and this
 * application is not in that path at all. It is exported so the regression
 * tests can assert the exact string and, more importantly, assert that
 * forwardCallback is never called with it.
 *
